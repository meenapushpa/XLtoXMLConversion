...
def index_tree():
    global root,tree,root3,root4,root5,root6,root7,root8
...
def street(arg1):
    ...
    newroot = etree.SubElement(root8,'TALLYMESSAGE')
...
def main():
    ...
    for index in check:
        ...
        global Totalamt,Totalotherchargesamnt,TotalSGST,TotalCGST,TotalRound
        Totalamt=0
        Totalotherchargesamnt=0
        TotalSGST=0
        TotalCGST=0
        TotalRound=0
        for counter, value in enumerate(ix):
            stramt=str(value['Amount'])
            strcgstamt=str(value['CGST Amount'])
            strsgstamt=str(value['SGST Amount'])
            strroundamt=str(value['Round off Amount'])
            otherchargesamnt=str(value['Other Charges_1 Amount'])
            if len(strroundamt) in(0,1):
                strroundamt='0.0'
            Totalamt =float(Totalamt)+float(stramt)+float(strcgstamt)+float(strsgstamt)+float(otherchargesamnt)+float(strroundamt)
        ...
        for counter, value in enumerate(ix):
            if len(index) <= 1:
                ...
                ledger_voucher(value['Customer Name'],ledgerpartyamount)
                b1.text=''
                ...
                if counter == 0:
                    ...
                    sub_other()
                    ledger_other('Ledger_Party')
                    stramount1=str(value['Amount'])
                    strroundamount1=str(value['Round off Amount'])
                    strqty=str(value['QTY']) + (value['UOM'])
                    strrate=str(value['Rate']) + '/' + (value['UOM'])
                    ledger_voucher(value['Customer Name'],ledgerpartyamount)
                    b1.text=''
                    if value['Other Charges_1 Amount'] != 0:
                        ledger_other('Ledger_Other1')
                        ledger_voucher(value['Other Charges_1 Ledger'],strOthercharges)
                        rate_gst(strOthercharges)
                    ledger_gst('CGST',strCGST,'Ledger_CGST',strCGST)
                    ledger_gst('SGST',strSGST,'Ledger_SGST',strSGST)
                    ledger_other('Ledger_Round')
                    ...
                    strcgstamount2=str(value['CGST Amount'])
                    strsgstamount2=str(value['SGST Amount'])
                    strroundamount2=str(value['Round off Amount'])
